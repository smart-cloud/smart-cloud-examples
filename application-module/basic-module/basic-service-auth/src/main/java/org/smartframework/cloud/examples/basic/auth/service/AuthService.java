 ...
 */
package org.smartframework.cloud.examples.basic.auth.service;

import com.baomidou.dynamic.datasource.annotation.DS;
import lombok.RequiredArgsConstructor;
import org.apache.commons.collections4.CollectionUtils;
import org.smartframework.cloud.examples.basic.auth.repository.PermissionInfoRepository;
import org.smartframework.cloud.examples.basic.auth.repository.RoleInfoRepository;
...
public class AuthService {
    ...
    public AuthRespDTO listByUid(Long uid) {
        Set<Long> roleIds = userRoleRepository.listRoleId(uid);
        if (CollectionUtils.isEmpty(roleIds)) {
            return new AuthRespDTO();
        }

        Set<String> roleCodes = roleInfoRepository.listCode(roleIds);
        if (CollectionUtils.isEmpty(roleCodes)) {
            return new AuthRespDTO();
        }

        Set<Long> permissionIds = rolePermissionRelaRepository.listPermissionIds(roleIds);
        Set<String> permissionCodes = null;
        if (CollectionUtils.isNotEmpty(permissionIds)) {
            permissionCodes = permissionInfoRepository.listCode(permissionIds);
        }
