 ...
 */
package org.smartframework.cloud.examples.basic.auth.repository;
...
public class RolePermissionRelaRepository extends BaseRepository<RolePermissionRelaMapper, RolePermissionRelaEntity> {
    ...
    public BasePageResponse<RolePermissionRespVO> page(PageRolePermissonReqVO req) {
        long count = getBaseMapper().countRole(req);
        ...
        if (count > 0) {
            ...
            List<RoleDO> roles = getBaseMapper().pageRole(pageRolePermissonReqDO);
            ...
            List<PermissionDO> permissions = getBaseMapper().listPermission(permissionReqDO);
